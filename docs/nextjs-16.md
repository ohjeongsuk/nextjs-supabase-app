...
> ⚠️ 이 프로젝트의 `next.config.ts`는 현재 `cacheComponents: true`만 설정되어 있고 `typedRoutes: true`는 **아직 활성화되어 있지 않습니다**. 아래 예시는 활성화할 경우의 사용법이며, 실제로 켜기 전까지는 `Link href`에 타입 체크가 적용되지 않습니다.
