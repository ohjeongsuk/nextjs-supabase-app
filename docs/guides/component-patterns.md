...
> 아래 예시(`UserAvatar`, `Accordion`, `VirtualizedList` 등)는 패턴 설명을 위한 일반적인 참고 코드이며 이 저장소에 실제로 존재하지 않습니다. 이 프로젝트에서 실제로 쓰이는 Server/Client Component 분리 예시는 `components/login-form.tsx`(Client, 폼 상호작용)와 `app/protected/page.tsx`(Server + `Suspense`)를 참고하세요.
