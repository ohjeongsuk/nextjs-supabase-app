...
- **TailwindCSS v3** (`^3.4.1`): 유틸리티 기반 CSS 프레임워크. `tailwind.config.ts` + `postcss.config.mjs` + `app/globals.css`의 `@tailwind base/components/utilities` 지시어 조합 (v4의 CSS 기반 설정이 아님)
...
- **CSS Variables**: 동적 테마 시스템 (`app/globals.css`의 `:root`/`.dark`)
- **class-variance-authority (cva)**, **tailwind-merge**, **clsx**: 변형 관리 및 클래스 병합 (`lib/utils.ts`의 `cn()`)
...
`app/globals.css`에 정의된 색상 변수 (neutral 계열, HSL 값):
...
다크모드 값(`.dark` 클래스 스코프)과 `--chart-*` 변수도 `app/globals.css`에 함께 정의되어 있으며, `tailwind.config.ts`가 이 변수들을 `hsl(var(--xxx))` 형태로 Tailwind 색상 토큰(`bg-primary`, `text-muted-foreground` 등)에 매핑합니다.
...
`tailwind.config.ts`의 `plugins: [require("tailwindcss-animate")]`로 등록되어 있어 별도 import 없이 유틸리티 클래스로 바로 사용합니다. shadcn/ui의 `DropdownMenu` 등 Radix 기반 컴포넌트가 열림/닫힘 애니메이션에 이 플러그인의 `animate-in`/`animate-out` 계열 클래스를 사용합니다 (`components/ui/dropdown-menu.tsx` 참고).

```tsx
// ✅ tailwindcss-animate의 진입/이탈 애니메이션
